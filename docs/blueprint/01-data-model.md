...
- **D6 Restock code:** assigned at finalize (drafts don't burn numbers), from the receipt date, race-safe via `@@unique([dateCode, seq])` + insert-retry.
- **D7 Reconciliation:** `varianceCents = receiptTotalCents − Σ lineTotalCents` stored at finalize. Auto-pass if `|variance| ≤ 2¢ × lineCount`; otherwise the UI requires an explicit acknowledgment (tax/fees are the usual cause). Purchaser bears the variance in v1; pro-rata tax allocation is a post-v1 door.
